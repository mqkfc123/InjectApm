...
namespace CInject
{
    public partial class frmMain : Form
    {
        ...
        private Type StartupInject = null;
        private MethodDefinition ProgramMethod = null;
        private MonoAssemblyResolver ProgramAssembly = null;
        ...
        private void InitializeUI()
        {
            ...
            if (directory != null)
            {
                ...
                {
                    if (file.Contains("grpc_csharp_ext"))
                        continue;
        ...
        private void runToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (BindMappings())
                {
                    ...
                    //_mapping.Add(new InjectionMapping(ProgramAssembly, ProgramMethod, StartupInject));

                    //var injection = new Injection
                    //{
                    //    InjectionType = StartupInject,
                    //    OnInvoke = ProgramAssembly.Assembly.ImportMethod(StartupInject, "OnInvoke"),
                    //    Constructor = ProgramAssembly.Assembly.ImportConstructor(StartupInject),
                    //    TypeReference = ProgramAssembly.Assembly.ImportType(StartupInject),
                    //    OnComplete = ProgramAssembly.Assembly.ImportMethod(StartupInject, "OnComplete")
                    //};
                    //ProgramAssembly.Assembly.MainModule.Import(StartupInject);
                    //ProgramAssembly.PatchMethod2(ProgramMethod, injection);
        ...
        private void AddInjectionAssembly(string assemblyName)
        {
            ...
            for (int i = 0; i < types.Count; i++)
            {
                List<Type> ts = assemblyInjectionCode.Assembly.GetType<Startup>();
            ...
            foreach (Type t in assemblyInjectionCode.Assembly.GetTypes())
            {
                ...
                if (t.Name.ToString() == "Startup")
                {
                    OnBindInjectors(assemblyInjectionCode, t, nodeAssembly, true);
